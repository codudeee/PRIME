...
module.exports = async function handler(req, res){
  try{
    ...
    const from = identity(body.from), to = identity(body.to);
    ...
    const fee = Math.floor(amount * 0.1), receive = amount - fee;
    ...
    return res.status(200).json({ok:true, amount, fee, receive, senderPrime:sender.after, receiverPrime:receiver.after});
  }catch(error){ return res.status(500).json({ok:false, message:error && error.message ? error.message : String(error)}); }
};
