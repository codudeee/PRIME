...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const result = await supabaseStore.readUserDocs({ limit, offset, q, discordId, tierOnly, cleanup: false });
      // GET은 조회만 수행한다. Discord 동기화/중복정리는 별도 PATCH action에서만 실행해서
      // 회원목록 로딩 중 row 삭제/지연/빈 목록이 생기지 않게 한다.
