...
const teamIndex = Number(slot.dataset.teamIndex);
  ...
  function resolveUserTierBadgeValue(user) {
    ...
    for (const field of getUserTierFields(user)) {
      ...
      if (/[0-5]\s*티어\s*[상중하]/.test(text) || /[0-5]\s*[상중하]/.test(text) || /^tier5[_-]?(high|mid|low)$/i.test(text) || /^beast[_-]?(high|mid|low)$/i.test(text)) return text;
      // generic 5티어/짐승은 세부 5상·5중·5하를 덮어쓰지 않는다.
      if (text === '짐승' || text === '5티어' || /^beast$/i.test(text) || /^tier5$/i.test(text)) continue;
...
function completeTeams() {
  ...
  function createSheetMemberFromSlot(teamIndex, slotIndex) {
    ...
    const specificTierCandidate = String(player.memberTier || player.__tierRole || player.tierRole || player.gradeRole || '').trim();
    const sourceTierCandidate = String(resolveUserTierBadgeValue(sourceUser) || '').trim();
    const generic5 = /^(tier5|5티어|beast|짐승)$/i;
    const memberTier = String(
      (specificTierCandidate && !generic5.test(specificTierCandidate) ? specificTierCandidate : '') ||
      sourceTierCandidate ||
      specificTierCandidate ||
      player.tier || ''
    ).trim();
    return {
      name: displayName,
      ...
      memberTier,
      __tierRole: memberTier,
      tierRole: memberTier,
      gradeRole: memberTier,
