...
const teamIndex = Number(slot.dataset.teamIndex);
  ...
  function bindUserSyncEvents() {
    window.addEventListener('storage', event => {
      if (event.key !== ADMIN_STORAGE_KEY && event.key !== ACCOUNT_STORAGE_KEY && event.key !== JOIN_RECRUIT_STATE_STORAGE_KEY && event.key !== JOIN_WAITLIST_STORAGE_KEY) return;
      if (event.key === JOIN_WAITLIST_STORAGE_KEY || event.key === JOIN_RECRUIT_STATE_STORAGE_KEY) {
        syncJoinWaitListIntoTeamBoard(true);
        syncPlayersWithUserSources();
      }
      hydratePlayersForDisplayOnly();
    ...
    window.addEventListener('pkl-join-state-updated', () => {
      syncJoinWaitListIntoTeamBoard(true);
      syncPlayersWithUserSources();
      hydratePlayersForDisplayOnly();
      renderTierPools();
