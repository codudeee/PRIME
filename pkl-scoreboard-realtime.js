(function(){
  ...
  function tierRole(text){var raw=clean(text), c=raw.replace(/[\s_-]+/g,'').toLowerCase();var map={tier0high:'tier0_high',tier0mid:'tier0_mid',tier0low:'tier0_low',tier1high:'tier1_high',tier1mid:'tier1_mid',tier1low:'tier1_low',tier2high:'tier2_high',tier2mid:'tier2_mid',tier2low:'tier2_low',tier3high:'tier3_high',tier3mid:'tier3_mid',tier3low:'tier3_low',tier4high:'tier4_high',tier4mid:'tier4_mid',tier4low:'tier4_low',beast:'beast_low',beasthigh:'beast_high',beastlow:'beast_low',beastmid:'beast_mid',tier5mid:'tier5_mid','0티어상':'tier0_high','0상':'tier0_high','0티어중':'tier0_mid','0중':'tier0_mid','0티어하':'tier0_low','0하':'tier0_low','1티어상':'tier1_high','1상':'tier1_high','1티어중':'tier1_mid','1중':'tier1_mid','1티어하':'tier1_low','1하':'tier1_low','2티어상':'tier2_high','2상':'tier2_high','2티어중':'tier2_mid','2중':'tier2_mid','2티어하':'tier2_low','2하':'tier2_low','3티어상':'tier3_high','3상':'tier3_high','3티어중':'tier3_mid','3중':'tier3_mid','3티어하':'tier3_low','3하':'tier3_low','4티어상':'tier4_high','4상':'tier4_high','4티어중':'tier4_mid','4중':'tier4_mid','4티어하':'tier4_low','4하':'tier4_low','5티어상':'tier5_high','5상':'tier5_high','5티어중':'tier5_mid','5중':'tier5_mid','5티어하':'tier5_low','5하':'tier5_low','짐승':'tier5_low','짐승상':'tier5_high','짐승하':'tier5_low','5상':'tier5_high','5하':'tier5_low','5티어상':'tier5_high','5티어하':'tier5_low','5티어 상':'tier5_high','5티어 하':'tier5_low'};return map[c]||raw;}
  ...
  function loadTierScoreConfig(){return fetch('/api/pkl-shared?key='+encodeURIComponent('pklTierScoreConfig'),{cache:'no-store',headers:{Accept:'application/json'}}).then(function(r){return r.json().catch(function(){return {};});}).then(function(data){var server=(data&&data.item&&data.item.value&&typeof data.item.value==='object')?data.item.value:{};var local={};try{local=JSON.parse(localStorage.getItem('pklTierScoreConfig')||'{}')||{};}catch(_e){}
    try{local=mergeTierScoreConfig(mergeTierScoreConfig({},local), JSON.parse(localStorage.getItem('PKL_TIER_SCORE_CONFIG_LOCAL_V2')||'{}')||{});}catch(_e){}
    window.__PKL_TIER_SCORE_CONFIG=mergeTierScoreConfig(mergeTierScoreConfig({},server),local);try{renderSnapshot(buildSnapshot());}catch(e){};}).catch(function(e){console.error('PKL scoreboard tier score config load failed',e);});}
