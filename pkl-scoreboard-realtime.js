(function(){
  ...
  function loadTierScoreConfig(){return fetch('/api/pkl-shared?key='+encodeURIComponent('pklTierScoreConfig'),{cache:'no-store',headers:{Accept:'application/json'}}).then(function(r){return r.json().catch(function(){return {};});}).then(function(data){var server=(data&&data.item&&data.item.value&&typeof data.item.value==='object')?data.item.value:{};var local={};try{local=JSON.parse(localStorage.getItem('pklTierScoreConfig')||'{}')||{};}catch(_e){}window.__PKL_TIER_SCORE_CONFIG=mergeTierScoreConfig(mergeTierScoreConfig({},server),local);try{renderSnapshot(buildSnapshot());}catch(e){};}).catch(function(e){console.error('PKL scoreboard tier score config load failed',e);});}
