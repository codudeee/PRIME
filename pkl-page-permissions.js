(function(){
  ...
  function applyPatch(){
    /* 패치노트 모달 내부 저장/취소 버튼까지 .patch-action-btn로 잡아 숨기면
       + 버튼으로 모달을 연 뒤 화면이 멈춘 것처럼 보인다.
       권한 제어는 바깥 작성/수정/삭제 버튼과 삭제 확인 버튼에만 적용한다. */
    qsa('#patchAddBtnSide,#patchEditBtn,#patchDeleteBtn,#patchConfirmDelete').forEach(function(el){
