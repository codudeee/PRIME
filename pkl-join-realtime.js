(function(){
  ...
  var applying=false, saveTimer=null, pollTimer=null, lastText='', lastRemoteMs=0, lastLocalEditAt=0, remoteReady=false, localChanged=false;
  ...
  function stateFromLocal(){var recruit=read(RECRUIT_KEY,{state:'loading'}); if(!remoteReady&&!localChanged){recruit={state:'loading'};} return {version:2,waitList:arr(read(WAIT_KEY,[])),cancelList:arr(read(CANCEL_KEY,[])),recruitState:recruit||{state:'loading'},updatedAt:now()};}
  ...
  function applyState(st){if(!st||typeof st!=='object')return; remoteReady=true;var ms=Date.parse(st.updatedAt||'')||0;if(ms&&ms<lastRemoteMs)return;if(Date.now()-lastLocalEditAt<650&&ms&&ms<Date.now()-650)return;lastRemoteMs=ms||Date.now();setLocal(WAIT_KEY,arr(st.waitList));setLocal(CANCEL_KEY,arr(st.cancelList));setLocal(RECRUIT_KEY,st.recruitState||{state:'loading'});lastText=textOf(st);emit(st);}
  ...
  if(!Storage.prototype.__pklJoinRealtimePatched){Storage.prototype.setItem=function(k,v){var ret=originalSet.apply(this,arguments);try{if(this===localStorage&&KEYS[String(k)]&&!applying){lastLocalEditAt=Date.now();localChanged=true;queueSave(String(k)===RECRUIT_KEY?350:500);if(!window.__PKL_JOIN_SUPPRESS_STORAGE_EMIT){emit(stateFromLocal());}}}catch(e){}return ret;};Storage.prototype.__pklJoinRealtimePatched=true;}
  try{Storage.prototype.removeItem=function(k){var ret=originalRemove.apply(this,arguments);try{if(this===localStorage&&KEYS[String(k)]&&!applying){lastLocalEditAt=Date.now();localChanged=true;queueSave(500);if(!window.__PKL_JOIN_SUPPRESS_STORAGE_EMIT){emit(stateFromLocal());}}}catch(e){}return ret;};}catch(e){}
