(function(){
  ...
  var applying=false, saveTimer=null, pollTimer=null, lastText='', lastRemoteMs=0, lastLocalEditAt=0, remoteReady=false, localChanged=false, currentState=null;
  ...
  function stateFromLocal(){var recruit=read(RECRUIT_KEY,{state:'loading'}); if(!remoteReady&&!localChanged){recruit={state:'loading'};} var st={version:2,waitList:arr(read(WAIT_KEY,[])),cancelList:arr(read(CANCEL_KEY,[])),recruitState:recruit||{state:'loading'},updatedAt:now()}; if(localChanged){currentState=st;} return st;}
  ...
  function applyState(st){if(!st||typeof st!=='object')return; remoteReady=true;var ms=Date.parse(st.updatedAt||'')||0;if(ms&&ms<lastRemoteMs)return;if(Date.now()-lastLocalEditAt<650&&ms&&ms<Date.now()-650)return;var normalized={version:2,waitList:arr(st.waitList),cancelList:arr(st.cancelList),recruitState:st.recruitState||{state:'loading'},updatedAt:st.updatedAt||now()};lastRemoteMs=ms||Date.now();currentState=normalized;setLocal(WAIT_KEY,normalized.waitList);setLocal(CANCEL_KEY,normalized.cancelList);setLocal(RECRUIT_KEY,normalized.recruitState);lastText=textOf(normalized);emit(normalized);}
  ...
  if(!Storage.prototype.__pklJoinRealtimePatched){Storage.prototype.setItem=function(k,v){var ret=originalSet.apply(this,arguments);try{if(this===localStorage&&KEYS[String(k)]&&!applying){lastLocalEditAt=Date.now();localChanged=true;queueSave(String(k)===RECRUIT_KEY?350:500);emit(stateFromLocal());}}catch(e){}return ret;};Storage.prototype.__pklJoinRealtimePatched=true;}
  try{Storage.prototype.removeItem=function(k){var ret=originalRemove.apply(this,arguments);try{if(this===localStorage&&KEYS[String(k)]&&!applying){lastLocalEditAt=Date.now();localChanged=true;queueSave(500);emit(stateFromLocal());}}catch(e){}return ret;};}catch(e){}
  /* 2차 청소: storage 이벤트 기반 자동 join 재렌더 금지. 클릭/저장 흐름에서만 emit한다. */
  window.PKLJoinRealtime={__pklJoinSupabase20260511:true,start:start,save:saveNow,queueSave:queueSave,state:stateFromLocal,getState:function(){return currentState||stateFromLocal();},apply:applyState,fetchNow:poll}; start();
})();
