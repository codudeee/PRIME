(function(){
  ...
  function mergeByIdentity(a,b){
    ...
    arr(a).concat(arr(b)).forEach(function(item){
      ...
      if(!out.some(function(prev){return same(prev,item);})) out.push(item);
    });
    return out;
  }
  function withoutMembers(list, remove){
    var rem=arr(remove);
    return arr(list).filter(function(item){return !rem.some(function(r){return same(r,item);});});
  }
  function applyState(st){
    ...
    if(nowMs-lastLocalEditAt<8000){
      ...
      if(localChanged && (!ms || ms<lastLocalEditAt)){
  ...
  function saveNow(){
    var st=stateFromLocal(), text=textOf(st);
    currentState=st;
    lastSaveAt=Date.parse(st.updatedAt||'')||Date.now();
    /* recent local edit은 lastText와 같아 보여도 서버에 한 번 더 확정 저장한다. */
